...
descriptor_info=$(bitcoin-cli getdescriptorinfo "tr($EXTENDED_KEY/0/*)")
descriptor=$(echo $descriptor_info | jq -r '.["descriptor"]')
addresses=$(bitcoin-cli deriveaddresses $descriptor "[0, 100]")
hundredth=$(echo $addresses | jq -r '.[100]')
